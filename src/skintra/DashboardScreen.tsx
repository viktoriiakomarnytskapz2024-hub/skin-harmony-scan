    ...
    <div className="relative min-h-full glow-bg pb-28">
      ...
      <div className="px-5 pt-12 pb-4 flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3">
          ...
          <div className="glass rounded-full px-3 py-1.5 text-xs font-ui font-medium text-foreground/80 flex items-center gap-1.5">
      ...
      <div className="px-5 mt-2 animate-slide-up">
        <p className="font-editorial text-base text-muted-foreground">Hello, Helen</p>
      ...
      <div className="px-5 mt-5 grid grid-cols-3 gap-2.5 animate-slide-up">
        {[
          ...
          <div key={s.label} className="glass rounded-2xl p-3 flex flex-col items-start gap-1">
            ...
            <p className="text-[10px] font-ui font-medium text-muted-foreground uppercase tracking-[0.18em]">{s.label}</p>
            <p className="font-display text-xl text-foreground leading-tight">{s.value}</p>
            <p className="text-[11px] font-editorial text-muted-foreground">{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="px-5 mt-5 animate-fade-in">
        <div className="glass rounded-3xl p-5">
          <div className="flex items-start justify-between">
            <div>
              <span className="inline-flex items-center gap-1 glass-soft rounded-full px-2.5 py-1 text-[10px] font-ui font-medium tracking-wider text-foreground/80">
                24 DEC
              </span>
              <h2 className="font-display text-3xl text-foreground leading-tight mt-1">
                Today's<br />routine
              </h2>
            </div>
            <button className="glass-soft rounded-full px-3 py-1.5 text-[11px] font-ui font-medium text-foreground inline-flex items-center gap-1">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div className="glass-inner p-3">
              <p className="text-[11px] font-editorial text-muted-foreground mb-2">0/4 Steps</p>
              <div className="grid grid-cols-2 gap-1.5">
                {[Droplet, Sparkles, FlaskConical, Sun].map((Icon, i) => (
                  <div
                    key={i}
                    className="aspect-square rounded-xl bg-white/40 border border-white/60 backdrop-blur-md flex items-center justify-center text-foreground/80"
                  >
            ...
            <div className="glass-inner p-3 flex flex-col items-center justify-center">
              <p className="text-[11px] font-editorial text-muted-foreground self-start">Skin Health</p>
              <div className="relative w-20 h-20 mt-1">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="font-display text-lg text-foreground">{skinHealth}%</span>
                </div>
              </div>
              <p className="text-[11px] font-editorial text-success font-semibold mt-1">+4% this week</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 inset-x-0 px-5 pb-6 pt-4 bg-gradient-to-t from-background via-background/95 to-transparent">
        <button
          onClick={onScan}
          className="w-full h-14 rounded-full text-primary-foreground font-ui font-semibold tracking-wide text-base flex items-center justify-center gap-2 shadow-[0_10px_30px_-8px_hsl(218_60%_50%/0.5)] active:scale-[0.98] transition"
